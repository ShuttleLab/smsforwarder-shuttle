...
const EN: Content = {
  ...
  metaDesc:
    "Automatically forward incoming SMS to your Telegram. SMS Forwarder is a free, self-hosted Android app — leave a phone plugged in at home and get verification codes and alerts anywhere.",
  ...
  howIntro: "The app is a thin client — it captures SMS and hands them to your backend, which does the routing:",
  howPoints: [
  ...
  boundaryH2: "Self-hosted and relay-and-forget",
  boundaryParas: [
  ...
  setupSteps: [
    { title: "Run the backend", desc: "Deploy the companion server (smsforwarder-api) — a single Go binary with SQLite — or point the app at a shared instance." },
    ...
    { title: "Sign in & bind Telegram", desc: "Create an account, then tap Bind Telegram, open the bot, and press Start. Your chat is now linked — no tokens to copy." },
  ...
  faqs: [
    { q: "Does it need my own server?", a: "Yes — SMS Forwarder is a thin client and needs a backend to relay messages. You can self-host the companion server (a single Go binary + SQLite, no external dependencies) or connect to a shared instance. Self-hosting keeps the whole path under your control and retains no message bodies after delivery." },
    ...
    { q: "Are my messages stored anywhere?", a: "The app reads incoming SMS only to forward them. On the backend it is relay-and-forget: a message body lives only until delivered, then it is deleted, leaving just a metadata log (result, time, sender). There are no ads, no analytics, and no tracking SDKs. If you self-host, no third party is ever in the loop." },
...
const ZH: Content = {
  ...
  metaDesc:
    "自动把收到的短信转发到你的 Telegram。短信转发是一款免费、自建的安卓应用——把手机插电放家里，验证码和通知随时随地都能收到。",
  h1: "安卓短信转发到 Telegram：随处接收你的短信",
  lead: "短信转发是一款免费的安卓应用，自动把手机收到的短信转发到你的 Telegram。它把每条短信上传到你自己的自建后端，由后端转发到 Telegram 后即删。把插着 SIM 卡的手机放家里插电常开，验证码、银行提醒、运营商短信就能随时随地找到你——不用漫游，也不用运营商的短信转移服务。",
  ...
  howH2: "短信转发如何送达你的短信",
  howIntro: "App 是瘦客户端——它负责捕获短信、交给你的后端，由后端做路由：",
  howPoints: [
    "捕获——短信广播接收器 + 前台服务轮询短信库双管齐下，用单调递增的 id 游标抓住每条新消息；即使在不下发短信广播的 ROM 上也不漏。",
    "上传——每条消息由活着的前台服务经 HTTPS 发往你的后端（而非会被系统限流的后台调度），并一直重试到成功。",
    "转发——后端通过共享 Bot 投递到你的 Telegram，随后删除正文；服务端幂等键保证同一条短信绝不重复投递。",
  ],
  boundaryH2: "自建 · 转发即焚",
  boundaryParas: [
    "短信转发在 App 内不保存任何 Telegram token，也不经由我们的云服务路由。小巧的配套后端（单个 Go 二进制 + SQLite）由你自己运行；正文只在投递完成前短暂驻留其中，投完即删，只给你（运营方）留一份元数据日志。",
    "登录使用可随时吊销的按设备令牌，你的 Telegram 绑定与账号关联，因此消息只会发给你。由于路由在服务端，以后加新渠道可在那里完成，无需重新编译或重装 App。",
  ],
  setupH2: "如何设置短信转发到 Telegram",
  setupSteps: [
    { title: "运行后端", desc: "部署配套服务器（smsforwarder-api）——单个 Go 二进制 + SQLite——或让 App 指向一个共享实例。" },
    { title: "安装 App", desc: "从 GitHub 下载 APK，装到插着 SIM 卡的那台手机上（Android 8.0 / API 26 及以上）。华为请先关闭「纯净模式」。" },
    { title: "登录并绑定 Telegram", desc: "注册账号，然后点「绑定 Telegram」，打开 Bot 点 Start。会话即绑定——无需复制任何 token。" },
  ...
  faqH2: "常见问题",
  faqs: [
    { q: "需要自己的服务器吗？", a: "需要——短信转发是瘦客户端，需要一个后端来中转消息。你可以自建配套服务器（单个 Go 二进制 + SQLite，无外部依赖），也可以连到共享实例。自建让整条链路都在你掌控之中，且投递后不保留任何正文。" },
    ...
    { q: "我的消息会被存起来吗？", a: "App 读取来信仅为转发。后端是「转发即焚」：正文只在投递完成前保留，之后即删，只留一份元数据日志（结果、时间、发件人）。没有广告、没有统计、没有追踪 SDK。若你自建，链路里不会有任何第三方。" },
